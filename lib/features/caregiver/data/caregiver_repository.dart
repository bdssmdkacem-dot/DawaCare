...
class CaregiverRepository {
  ...
  Future<List<CaregiverLink>> fetchLinkedPatients(String caregiverId) async {
    final rows = await _client.from('caregiver_patient').select('*, patient:profiles!patient_id(full_name, avatar_url)').eq('caregiver_id', caregiverId).order('created_at');
  ...
  Future<FamilyMemberSummary> fetchMemberSummary(String patientId) async {
    ...
    final results = await Future.wait([
      ...
      _client.from('dose_instances').select('*').eq('patient_id', patientId).gte('scheduled_at', start.toUtc().toIso8601String()).lt('scheduled_at', end.toUtc().toIso8601String()).order('scheduled_at'),
    ]);
    final medicationRows = results[0] as List;
    final doseRows = results[1] as List;
    final doses = doseRows.map((row) => DoseInstance.fromMap(Map<String, dynamic>.from(row as Map))).toList();
    return FamilyMemberSummary.fromDoses(activeMedicationCount: medicationRows.length, todayDoses: doses);
  }

  Future<void> unlink(String linkId) async => _client.from('caregiver_patient').delete().eq('id', linkId);
  ...
  Future<String> requestLink({required String code, required CaregiverRole role, String? relationshipLabel}) async {
    try {
      final rows = await _client.rpc('request_family_link', params: {'p_code': _normalizeLinkCode(code), 'p_relationship_label': relationshipLabel, 'p_role': caregiverRoleToDb(role)});
      final row = (rows as List).first as Map<String, dynamic>;
      final requestId = row['request_id'] as String?;
      if (requestId != null && requestId.isNotEmpty) {
        try { await _notifyFamilyLink(requestId, 'REQUESTED'); } catch (_) {}
      }
  ...
  Future<void> cancelRequest(String requestId) async {
    try { await _client.rpc('cancel_family_link_request', params: {'p_request_id': requestId}); }
    on PostgrestException catch (e) { throw FamilyLinkException(_extractCode(e.message)); }
  }
  ...
  Future<String> respondToRequest({required String requestId, required bool approve}) async {
    try {
      ...
      try { await _notifyFamilyLink(requestId, approve ? 'APPROVED' : 'REJECTED'); } catch (_) {}
      return status;
    } on PostgrestException catch (e) {
      throw FamilyLinkException(_extractCode(e.message));
    }
  ...
  Future<void> _notifyFamilyLink(String requestId, String eventType) async {
    final response = await _client.functions.invoke('family-link-notify', body: {'request_id': requestId, 'event_type': eventType});
    if (response.status < 200 || response.status >= 300) throw StateError('family-link-notify failed: ${response.status}');
  }
  ...
  Future<void> markAlertRead(String alertId) async => _client.from('caregiver_alerts').update({'read': true}).eq('id', alertId);
  ...
  String _normalizeLinkCode(String value) {
    ...
    for (var i = 0; i < latin.length; i++) result = result.replaceAll(arabicIndic[i], latin[i]).replaceAll(extendedArabicIndic[i], latin[i]);
    return result.replaceAll(RegExp(r'[^0-9]'), '');
  }

  String _extractCode(String message) => RegExp(r'[A-Z_]{6,}').firstMatch(message)?.group(0) ?? 'UNKNOWN_ERROR';
}
