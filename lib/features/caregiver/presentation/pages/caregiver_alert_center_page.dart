...
class CaregiverAlertCenterPage extends StatelessWidget {
  ...
  Future<void> _openAlert(BuildContext context, CaregiverAlert alert) async {
    ...
    final link = caregiver.linkedPatients.cast<CaregiverLink?>().firstWhere((item) => item?.patientId == alert.patientId, orElse: () => null);
    if (link == null) return;

    final kind = _kind(alert);
    if (kind == _AlertKind.lowStock || kind == _AlertKind.outOfStock) {
      final medications = MedicationProvider();
      try {
        await medications.load(alert.patientId);
        Medication? target;
        if (alert.medicationId != null) {
          target = medications.medications.cast<Medication?>().firstWhere((m) => m?.id == alert.medicationId, orElse: () => null);
        }
        target ??= _matchMedication(medications.medications, alert.message);
        if (!context.mounted) return;
        if (target != null) {
          await Navigator.of(context).push(MaterialPageRoute(builder: (_) => CaregiverMedicationRefillPage(medication: target!, patientName: link.patientName, canManageStock: PatientPermissions.forRole(link.role).canManageStock)));
          return;
        }
      } finally {
        medications.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: provider.isLoading && provider.alerts.isEmpty ? const LoadingIndicator() : ListView(
          ...
          children: [
            ...
            if (provider.alerts.isEmpty) Card(child: Padding(padding: const EdgeInsets.all(28), child: Column(children: [Icon(Icons.notifications_off_rounded, size: 48, color: AppColors.primary.withValues(alpha: .65)), const SizedBox(height: 12), Text(_tr(context, 'لا توجد تنبيهات تحتاج إلى تدخل الآن.', 'No alerts need your attention right now.', 'Aucune alerte ne nécessite votre intervention.'), textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w700))]))),
          ],
        ),
      ),
    );
  ...
  Widget _summary(BuildContext context, CaregiverProvider provider) => Card(child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
  ...
  Widget _alertTile(BuildContext context, CaregiverAlert alert) {
    final kind = _kind(alert);
    final icon = switch (kind) { _AlertKind.missed => Icons.schedule_rounded, _AlertKind.lowStock => Icons.inventory_rounded, _AlertKind.outOfStock => Icons.production_quantity_limits_rounded, _AlertKind.other => Icons.notifications_rounded };
    final title = switch (kind) { _AlertKind.missed => _tr(context, 'جرعة فائتة', 'Missed dose', 'Dose manquée'), _AlertKind.lowStock => _tr(context, 'المخزون منخفض', 'Low stock', 'Stock faible'), _AlertKind.outOfStock => _tr(context, 'المخزون نفد', 'Out of stock', 'Rupture de stock'), _AlertKind.other => alert.type };
    final stock = kind == _AlertKind.lowStock || kind == _AlertKind.outOfStock;
    return Card(margin: const EdgeInsets.only(bottom: 8), child: InkWell(borderRadius: BorderRadius.circular(16), onTap: () => _openAlert(context, alert), child: Padding(padding: const EdgeInsets.all(13), child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Expanded(child: Text(title, style: const TextStyle(fontWeight: FontWeight.w900))), if (!alert.read) Container(width: 8, height: 8, decoration: const BoxDecoration(shape: BoxShape.circle, color: AppColors.primary))]), const SizedBox(height: 3), Text(alert.patientName, style: const TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 2), Text(alert.message, maxLines: 2, overflow: TextOverflow.ellipsis), if (stock) ...[const SizedBox(height: 7), Row(children: [Icon(Icons.arrow_forward_rounded, size: 16, color: AppColors.primary), const SizedBox(width: 5), Text(_tr(context, 'فتح الدواء وإدارة المخزون', 'Open medication & manage stock', 'Ouvrir le médicament et gérer le stock'), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800))])])),
  ...
  String _tr(BuildContext context, String ar, String en, String fr) { switch (Localizations.localeOf(context).languageCode) { case 'en': return en; case 'fr': return fr; default: return ar; } }
}
