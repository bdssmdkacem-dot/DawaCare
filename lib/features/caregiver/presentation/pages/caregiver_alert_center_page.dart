...
class CaregiverAlertCenterPage extends StatelessWidget {
      ...
      try {
        ...
        if (target != null) {
          await Navigator.of(context).push(
            MaterialPageRoute(
              builder: (_) => CaregiverMedicationRefillPage(
                medication: target!,
                patientName: link.patientName,
                canManageStock:
                    PatientPermissions.forRole(link.role).canManageStock,
              ),
            ),
          );
          return;
  ...
  Future<void> _refresh(BuildContext context) async {
    ...
    if (userId != null) {
      await context.read<CaregiverProvider>().load(userId);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final missed = provider.alerts
        .where((a) => _kind(a) == _AlertKind.missed)
        .toList();
    final low = provider.alerts
        .where((a) => _kind(a) == _AlertKind.lowStock)
        .toList();
    final out = provider.alerts
        .where((a) => _kind(a) == _AlertKind.outOfStock)
        .toList();
    final other = provider.alerts
        .where((a) => _kind(a) == _AlertKind.other)
        .toList();
        ...
        child: provider.isLoading && provider.alerts.isEmpty
            ...
            : ListView(
                ...
                children: [
                  ...
                  if (other.isNotEmpty)
                    _section(context, l, _AlertKind.other, other),
                        ...
                        child: Column(
                          children: [
                            ...
                            Text(
                              _tr(
                                context,
                                'لا توجد تنبيهات تحتاج إلى تدخل الآن.',
                                'No alerts need your attention right now.',
                                'Aucune alerte ne nécessite votre intervention.',
                              ),
  ...
  Widget _summary(BuildContext context, CaregiverProvider provider) {
    return Card(
      child: Padding(
        ...
        child: Row(
          children: [
            _metric(
              ...
              _tr(context, 'غير مقروء', 'Unread', 'Non lues'),
            ),
            _divider(),
            _metric(
              context,
              Icons.medication_rounded,
              '${provider.lowStockMedicationCount}',
              _tr(context, 'مخزون منخفض', 'Low stock', 'Stock faible'),
            ),
            _divider(),
            _metric(
              context,
              Icons.inventory_2_rounded,
              '${provider.outOfStockMedicationCount}',
              _tr(context, 'نفد المخزون', 'Out of stock', 'Rupture'),
            ),
          ],
        ),
      ),
  ...
  Widget _section(
    ...
    final title = switch (kind) {
      _AlertKind.missed =>
        _tr(context, 'جرعات فائتة', 'Missed doses', 'Doses manquées'),
      _AlertKind.lowStock =>
        _tr(context, 'مخزون منخفض', 'Low stock', 'Stock faible'),
      _AlertKind.outOfStock =>
        _tr(context, 'نفاد المخزون', 'Out of stock', 'Rupture de stock'),
  ...
  Widget _alertTile(BuildContext context, CaregiverAlert alert) {
    ...
    final title = switch (kind) {
      _AlertKind.missed =>
        _tr(context, 'جرعة فائتة', 'Missed dose', 'Dose manquée'),
      _AlertKind.lowStock =>
        _tr(context, 'المخزون منخفض', 'Low stock', 'Stock faible'),
      _AlertKind.outOfStock =>
        _tr(context, 'المخزون نفد', 'Out of stock', 'Rupture de stock'),
      _AlertKind.other => alert.type,
    };

    final stock =
        kind == _AlertKind.lowStock || kind == _AlertKind.outOfStock;
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              _tr(
                                context,
                                'فتح الدواء وإدارة المخزون',
                                'Open medication & manage stock',
                                'Ouvrir le médicament et gérer le stock',
                              ),
                              style: const TextStyle(
                                fontSize: 12,
  ...
  static _AlertKind _kind(CaregiverAlert alert) {
    ...
    if (value.contains('OUT_OF_STOCK') ||
        value.contains('OUT OF STOCK') ||
        value.contains('DEPLETED') ||
        value.contains('نفد') ||
        value.contains('نفاذ')) {
      return _AlertKind.outOfStock;
    }
    if (value.contains('LOW_STOCK') ||
        value.contains('LOW STOCK') ||
        value.contains('LOW') ||
        value.contains('منخفض')) {
      return _AlertKind.lowStock;
    }
    if (value.contains('MISSED') ||
        value.contains('MISS') ||
        value.contains('فائت') ||
        value.contains('فائتة')) {
      return _AlertKind.missed;
    }
