...
class _PatientDetailPageState extends State<PatientDetailPage> {
  ...
  Widget _patientHero(BuildContext context) {
    ...
    return Container(
      ...
      child: Row(children: [
        Container(width: 64, height: 64, decoration: BoxDecoration(color: Colors.white.withValues(alpha: .14), shape: BoxShape.circle), child: const Icon(Icons.person_rounded, color: Colors.white, size: 34)),
  ...
  Widget _doseCard(DoseInstance dose) => Padding(
    padding: const EdgeInsets.only(bottom: 10),
    child: DoseCard(
      dose: dose,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: p.isLoading && p.all.isEmpty ? const LoadingIndicator() : RefreshIndicator(
        ...
        child: ListView(padding: const EdgeInsets.fromLTRB(16, 14, 16, 32), children: [
          ...
          if (highlightedDose != null) ...[
            _sectionHeader(context, _tr(context, 'الجرعة المرتبطة بالتنبيه', 'Dose linked to alert', 'Dose liée à l’alerte'), icon: Icons.warning_amber_rounded),
            Card(color: AppColors.danger.withValues(alpha: .055), child: Padding(padding: const EdgeInsets.fromLTRB(12, 12, 12, 2), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(_tr(context, 'هذه هي الجرعة المرتبطة بهذا التنبيه.', 'This is the dose linked to this alert.', 'Voici la dose liée à cette alerte.'), style: Theme.of(context).textTheme.bodySmall),
          ...
          const SizedBox(height: 18),
          Card(color: AppColors.primary.withValues(alpha: .055), child: InkWell(
            borderRadius: BorderRadius.circular(20),
            onTap: () => _openVoiceRecorder(),
            child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
              Container(width: 48, height: 48, decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: .12), shape: BoxShape.circle), child: const Icon(Icons.mic_rounded, color: AppColors.primary)),
              const SizedBox(width: 13),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(l.sendGeneralVoice, style: const TextStyle(fontWeight: FontWeight.w800)),
                const SizedBox(height: 3),
                Text(_tr(context, 'أرسل رسالة صوتية عامة لهذا الشخص.', 'Send a general voice message to this person.', 'Envoyez un message vocal général à cette personne.'), style: Theme.of(context).textTheme.bodySmall),
              ])),
          ...
          if (p.todayDoses.isEmpty)
            Card(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 24), child: Column(children: [
              Container(width: 58, height: 58, decoration: BoxDecoration(color: AppColors.primary.withValues(alpha: .09), shape: BoxShape.circle), child: const Icon(Icons.medication_outlined, color: AppColors.primary, size: 29)),
              const SizedBox(height: 12),
              Text(l.noScheduledMedicines, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w700)),
            ])))
          else
            ...todayDoses.map(_doseCard),
          const SizedBox(height: 10),
          Center(child: TextButton.icon(onPressed: () => _confirmUnlink(context), icon: const Icon(Icons.link_off_rounded, color: AppColors.danger), label: Text(l.removeLink, style: const TextStyle(color: AppColors.danger, fontWeight: FontWeight.w700)))),
        ]),
      ),
    );
  }
  ...
  Future<void> _confirmUnlink(BuildContext context) async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
      ...
      actions: [TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text(l.cancel)), TextButton(onPressed: () => Navigator.pop(ctx, true), child: Text(l.removeLink))],
    ));
