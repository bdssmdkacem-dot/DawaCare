...
class _PatientDetailPageState extends State<PatientDetailPage> {
  ...
  PatientPermissions get _permissions => PatientPermissions.forRole(widget.link.role);
  ...
  void _openMedication(Medication medication) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => CaregiverMedicationDetailPage(
      ...
      patientName: widget.link.patientName,
      canManageDoses: _permissions.canManageDose,
    )));
  }
  ...
  DoseInstance? _nextDose(List<DoseInstance> doses) {
    final now = DateTime.now();
    final upcoming = doses.where((d) => d.scheduledAt.isAfter(now) &&
        d.status != DoseStatus.cancelled && d.status != DoseStatus.skipped).toList();
    if (upcoming.isEmpty) return null;
    upcoming.sort((a, b) => a.scheduledAt.compareTo(b.scheduledAt));
    return upcoming.first;
  }

  Widget _sectionHeader(BuildContext context, String title, {required IconData icon}) {
    return Padding(
  ...
  Widget _patientHero(BuildContext context) {
    final role = _roleLabel(context, widget.link.role);
  ...
  Future<void> _showProfile(BuildContext context) async {
    ...
    await showDialog<void>(context: context, builder: (dialogContext) => Dialog(
      ...
      child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 8), Text(_roleLabel(context, widget.link.role)),
  ...
  Widget _reportsCard(BuildContext context) {
    return Card(margin: const EdgeInsets.only(bottom: 4), child: InkWell(
      borderRadius: BorderRadius.circular(16), onTap: _openReports,
  ...
  Widget _medicationTile(BuildContext providerContext, Medication medication) {
    ...
    return Card(margin: const EdgeInsets.only(bottom: 9), child: InkWell(
      borderRadius: BorderRadius.circular(16), onTap: () => _openMedication(medication),
  ...
  Widget build(BuildContext context) {
    ...
    return MultiProvider(
      providers: [ChangeNotifierProvider<DoseProvider>(create: (_) => DoseProvider()), ChangeNotifierProvider<MedicationProvider>(create: (_) => MedicationProvider())],
      child: Consumer2<DoseProvider, MedicationProvider>(
        builder: (providerContext, p, medications, _) {
          if (!_loadScheduled) {
            _loadScheduled = true;
            WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) _loadPatientData(providerContext); });
          }
          ...
          final stats = AdherenceCalculator.compute(p.all);
          final nextDose = _nextDose(p.all);
          final lowStockCount = medications.medications.where((m) => StockIntelligence.isLowStock(m)).length;
          final outOfStockCount = medications.medications.where((m) => StockIntelligence.isOutOfStock(m)).length;
                    ...
                    child: ListView(
                      ...
                      children: [
                        ...
                        PatientStatusHeader(
                          ...
                          tr: (ar, en, fr) => _tr(providerContext, ar, en, fr),
                          onNextDose: nextDose == null ? null : () => _doseCard(providerContext, nextDose),
                        ...
                        Card(child: Padding(padding: const EdgeInsets.all(16), child: AdherenceChart(stats: stats))),
                        const SizedBox(height: 18),
  ...
  Future<void> _confirmUnlink(BuildContext context) async {
    if (!_permissions.canUnlink) return;
    final l = AppLocalizations.of(context);
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(
      ...
      actions: [
        ...
        FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: Text(l.removeLink)),
      ],
    ));
    if (confirmed != true || !mounted) return;
    try { await caregiverProvider.unlink(widget.link); } catch (_) {}
