...
class FamilyMemberCard extends StatelessWidget {
  ...
  Widget _summary(BuildContext context) {
    ...
    final data = summary ?? const FamilyMemberSummary.empty();
    final adherence = (data.adherence * 100).round();
    return Column(
      children: [
        const Divider(height: 20),
        Row(children: [
          _metric(context, Icons.medication_outlined, '${data.activeMedicationCount}', _tr(context, 'أدوية نشطة', 'Active meds', 'Médicaments actifs')),
          _metric(context, Icons.today_rounded, '${data.todayDoseCount}', _tr(context, 'جرعات اليوم', 'Today', 'Aujourd’hui')),
          _metric(context, Icons.check_circle_outline_rounded, '$adherence%', _tr(context, 'الالتزام', 'Adherence', 'Observance')),
        ]),
        const SizedBox(height: 10),
        Row(children: [
          Icon(data.missedDoseCount > 0 ? Icons.warning_amber_rounded : Icons.verified_rounded, size: 17, color: data.missedDoseCount > 0 ? Colors.orange : AppColors.primary),
          const SizedBox(width: 6),
          Expanded(child: Text(
            data.missedDoseCount > 0
                ? _tr(context, '${data.missedDoseCount} جرعات فائتة اليوم', '${data.missedDoseCount} missed doses today', '${data.missedDoseCount} doses manquées aujourd’hui')
