...
class PatientStatusHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final adherence = dueStats == 0 ? 0 : stats.takenPercent;
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(
                  critical ? Icons.warning_rounded : Icons.health_and_safety_rounded,
                  color: critical ? AppColors.danger : AppColors.primary,
                ),
                const SizedBox(width: 9),
                Expanded(
                  child: Text(
                    tr('الحالة اليومية', 'Daily status', 'État quotidien'),
                    style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w900),
                  ),
                ),
                if (onReports != null)
                  IconButton(
                    tooltip: tr('التقارير', 'Reports', 'Rapports'),
                    onPressed: onReports,
            ...
            Row(
              children: [
                ...
                Expanded(child: _metric('${lowStockCount + outOfStockCount}', tr('مخزون', 'Stock risk', 'Risque stock'), danger: lowStockCount + outOfStockCount > 0)),
              ],
            ),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            tr('الجرعة القادمة', 'Next dose', 'Prochaine dose'),
              ...
              Wrap(
                ...
                children: [
                  if (outOfStockCount > 0)
                    _riskChip(Icons.production_quantity_limits_rounded, '${tr('نفاد', 'Out', 'Épuisé')} $outOfStockCount'),
                  if (lowStockCount > 0)
                    _riskChip(Icons.warning_amber_rounded, '${tr('منخفض', 'Low', 'Faible')} $lowStockCount'),
                ],
              ),
            ],
          ],
  ...
  Widget _metric(String value, String label, {bool danger = false}) {
    return Column(
      children: [
  ...
  Widget _riskChip(IconData icon, String text) {
    return Container(
