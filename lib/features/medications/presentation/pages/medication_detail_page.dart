...
class _MedicationDetailPageState extends State<MedicationDetailPage> {
          ...
          children: [
            ...
            FutureBuilder<List<DoseInstance>>(
              ...
              builder: (context, snapshot) {
                ...
                final doses = (snapshot.data ?? const <DoseInstance>[])
                ...
                return Container(
                  ...
                  child: url == null
                      ...
                      : Image.network(
                          url,
                          fit: BoxFit.cover,
                          errorBuilder: (_, __, ___) => const Icon(Icons.medication_liquid_rounded, color: AppColors.primary, size: 42),
                        ),
                );
              },
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (med.genericName != null && med.genericName!.trim().isNotEmpty) ...[
                    ...
                    Text(med.genericName!.trim(), style: theme.textTheme.bodyMedium),
                  ],
                  ...
                  Wrap(
                    ...
                    runSpacing: 6,
                    children: [
                      if (med.strength != null && med.strength!.trim().isNotEmpty) _tag(context, med.strength!.trim()),
                      if (med.dosageForm != null && med.dosageForm!.trim().isNotEmpty)
                        _tag(context, AppLocalizations.of(context).dosageFormLabel(med.dosageForm!.trim())),
                      _tag(context, _tr('نشط', 'Active', 'Actif')),
                    ],
                  ),
                ],
              ),
  ...
  Widget _scheduleCard(BuildContext context, MedicationSchedule schedule) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(DoseEngineLabel.describe(schedule, l), style: const TextStyle(fontWeight: FontWeight.w800)),
        subtitle: Padding(
          padding: const EdgeInsets.only(top: 4),
          child: Text('${l.doseAmount}: ${schedule.doseAmount}'),
        ),
      ),
    );
  }
      ...
      child: Column(
        children: [
          ListTile(
            ...
            trailing: Text(DateTimeUtils.formatShortDate(med.startDate), style: const TextStyle(fontWeight: FontWeight.w800)),
          ),
          ListTile(
            leading: const Icon(Icons.event_busy_rounded, color: AppColors.primary),
            title: Text(l.endDateOptional),
            trailing: Text(med.endDate == null ? '—' : DateTimeUtils.formatShortDate(med.endDate!), style: const TextStyle(fontWeight: FontWeight.w800)),
          ),
        ],
      ),
    );
  ...
  Widget _infoCard(BuildContext context, IconData icon, String text) {
    return Card(child: ListTile(leading: Icon(icon, color: AppColors.primary), title: Text(text)));
  }
...
class DoseEngineLabel {
  static String describe(MedicationSchedule schedule, AppLocalizations l) {
    switch (schedule.type) {
      case ScheduleType.daily:
        return '${l.daily} — ${schedule.time}';
      case ScheduleType.weekly:
      case ScheduleType.specificDays:
        final days = schedule.daysOfWeek.map(l.weekdayLabel).join('، ');
