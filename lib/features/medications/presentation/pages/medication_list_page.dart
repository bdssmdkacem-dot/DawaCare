...
class _MedicationListPageState extends State<MedicationListPage> {
  ...
  Future<void> _confirmDeactivate(Medication medication) async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
      title: Text(l.deactivateMedicineTitle), content: Text(l.deactivateMedicineBody(medication.name)),
...
class _MedicationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(child: ListTile(
      ...
      leading: FutureBuilder<String?>(future: imageUrlFuture, builder: (context, snapshot) {
        if (snapshot.data == null) return CircleAvatar(backgroundColor: theme.colorScheme.primary.withValues(alpha: 0.12), child: Icon(Icons.medication_liquid_rounded, color: theme.colorScheme.primary));
        return ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.network(snapshot.data!, width: 58, height: 58, fit: BoxFit.cover, errorBuilder: (_, __, ___) => CircleAvatar(backgroundColor: theme.colorScheme.primary.withValues(alpha: 0.12), child: Icon(Icons.medication_liquid_rounded, color: theme.colorScheme.primary)));
      }),
      title: Text(medication.name, style: const TextStyle(fontWeight: FontWeight.w700)),
      subtitle: Text([if (medication.strength != null && medication.strength!.isNotEmpty) medication.strength!, if (scheduleLabel != null) scheduleLabel!].join(' · ')),
      trailing: PopupMenuButton<String>(
        onSelected: (value) { if (value == 'change_image') onChangeImage(); if (value == 'remove_image') onRemoveImage?.call(); if (value == 'deactivate') onDeactivate(); },
        itemBuilder: (context) => [
