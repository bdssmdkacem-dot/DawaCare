...
class _MedicationListPageState extends State<MedicationListPage> {
    ...
    final quantity = await showDialog<double>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () { final value = double.tryParse(controller.text.trim().replaceAll(',', '.')); if (value != null && value > 0) Navigator.pop(ctx, value); }, child: const Text('حفظ')),
        ],
      ),
    );
    controller.dispose();
  ...
  bool _matches(Medication medication) {
    ...
    if (q.isNotEmpty && !('${medication.name} ${medication.genericName ?? ''} ${medication.strength ?? ''} ${medication.dosageForm ?? ''}'.toLowerCase().contains(q))) return false;
    switch (_filter) {
      case _MedicationFilter.all: return true;
      case _MedicationFilter.lowStock: return medication.stockEnabled && medication.stockQuantity > 0 && medication.stockQuantity <= medication.lowStockThreshold;
      case _MedicationFilter.outOfStock: return medication.stockEnabled && medication.stockQuantity <= 0;
  ...
  Widget _buildBody(MedicationProvider provider, AppLocalizations l) {
    ...
    final low = provider.medications.where((m) => m.stockEnabled && m.stockQuantity > 0 && m.stockQuantity <= m.lowStockThreshold).length;
    final empty = provider.medications.where((m) => m.stockEnabled && m.stockQuantity <= 0).length;
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          _MedicationOverview(total: provider.medications.length, low: low, empty: empty),
          const SizedBox(height: 12),
          TextField(
            onChanged: (value) => setState(() => _query = value),
            decoration: InputDecoration(prefixIcon: const Icon(Icons.search_rounded), hintText: 'ابحث باسم الدواء أو التركيز أو الشكل', suffixIcon: _query.isEmpty ? null : IconButton(onPressed: () { setState(() => _query = ''); }, icon: const Icon(Icons.clear_rounded))),
          ),
          const SizedBox(height: 10),
          SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: Row(children: [
              _filterChip('الكل', _MedicationFilter.all, provider.medications.length),
          ...
          const SizedBox(height: 12),
          if (visible.isEmpty)
            Card(child: Padding(padding: const EdgeInsets.all(24), child: Column(children: [const Icon(Icons.search_off_rounded, size: 42), const SizedBox(height: 10), Text('لا توجد أدوية مطابقة', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)), const SizedBox(height: 5), const Text('جرّب تغيير البحث أو الفلتر.')])) )
          else
            ...visible.map((med) {
              final schedules = provider.schedulesByMedicationId[med.id] ?? const <MedicationSchedule>[];
              return Padding(
                padding: const EdgeInsets.only(bottom: 10),
                child: _MedicationTile(medication: med, schedules: schedules, imageUrlFuture: provider.signedMedicationImageUrl(med.imageUrl), onTap: () => _openDetails(med), onChangeImage: () => _changeImage(med), onRemoveImage: med.imageUrl == null ? null : () => _removeImage(med), onDeactivate: () => _confirmDeactivate(med), onAddStock: () => _addStock(med), onStockDetails: () => _openStockDetails(med)),
              );
            }),
        ],
      ),
...
class _MedicationOverview extends StatelessWidget {
  ...
  final int low;
  final int empty;
  const _MedicationOverview({required this.total, required this.low, required this.empty});
  ...
  Widget build(BuildContext context) {
    return Card(child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
      ...
      _divider(),
      Expanded(child: _metric(context, Icons.warning_amber_rounded, '$low', 'منخفض')),
      _divider(),
      Expanded(child: _metric(context, Icons.error_outline_rounded, '$empty', 'نفد')),
    ])));
  }

  Widget _metric(BuildContext context, IconData icon, String value, String label) => Column(children: [Icon(icon, color: AppColors.primary, size: 23), const SizedBox(height: 5), Text(value, style: const TextStyle(fontSize: 19, fontWeight: FontWeight.w900)), Text(label, style: Theme.of(context).textTheme.bodySmall)];
  Widget _divider() => Container(width: 1, height: 42, margin: const EdgeInsets.symmetric(horizontal: 4), color: Theme.of(context).dividerColor);
}
...
class _MedicationTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(child: InkWell(borderRadius: BorderRadius.circular(20), onTap: onTap, child: Padding(padding: const EdgeInsets.fromLTRB(14, 12, 8, 12), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      FutureBuilder<String?>(future: imageUrlFuture, builder: (context, snapshot) {
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 5, runSpacing: 4, children: [if (medication.strength != null && medication.strength!.trim().isNotEmpty) _chip(medication.strength!.trim()), if (medication.dosageForm != null && medication.dosageForm!.trim().isNotEmpty) _chip(l.dosageFormLabel(medication.dosageForm!.trim())), if (schedules.isNotEmpty) _chip(_scheduleCountLabel(l, schedules.length))]),
        if (schedules.isNotEmpty) ...[
          const SizedBox(height: 6),
          ...schedules.take(2).map((s) => Padding(padding: const EdgeInsets.only(bottom: 2), child: Text('• ${DoseEngine.describeSchedule(s)} · ${l.doseAmount}: ${s.doseAmount}', maxLines: 1, overflow: TextOverflow.ellipsis, style: theme.textTheme.bodySmall))),
