...
class _MedicationStockDetailPageState extends State<MedicationStockDetailPage> {
  ...
  bool _loading = true;
  String? _error;
  final GlobalKey<MedicationStockHistoryState> _historyKey = GlobalKey<MedicationStockHistoryState>();
  ...
  Future<void> _addStock() async {
    ...
    final result = await showDialog<double>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(controller: controller, keyboardType: const TextInputType.numberWithOptions(decimal: true), decoration: InputDecoration(labelText: 'الكمية (${_medication.stockUnit})')),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('إلغاء')),
          FilledButton(onPressed: () { final value = double.tryParse(controller.text.trim().replaceAll(',', '.')); Navigator.pop(ctx, value); }, child: const Text('إضافة')),
        ],
      ),
    );
    controller.dispose();
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
            DropdownButtonFormField<String>(initialValue: unit, decoration: const InputDecoration(labelText: 'الوحدة'), items: const [
              DropdownMenuItem(value: 'unit', child: Text('وحدة')), DropdownMenuItem(value: 'tablet', child: Text('قرص')), DropdownMenuItem(value: 'capsule', child: Text('كبسولة')), DropdownMenuItem(value: 'ml', child: Text('مل')), DropdownMenuItem(value: 'drop', child: Text('قطرة')), DropdownMenuItem(value: 'injection', child: Text('حقنة')),
            ], onChanged: (value) { if (value != null) setDialogState(() => unit = value); }),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(_medication.name), actions: [IconButton(onPressed: _loading ? null : _load, icon: const Icon(Icons.refresh))]),
      body: _loading ? const Center(child: CircularProgressIndicator()) : _error != null ? Center(child: Text(_error!)) : RefreshIndicator(
        ...
        child: ListView(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.all(16), children: [
          Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 12), Text('${_formatNumber(_medication.stockQuantity)} ${_medication.stockUnit}', style: Theme.of(context).textTheme.headlineMedium),
            const SizedBox(height: 12), _InfoRow(label: 'حد المخزون المنخفض', value: '${_formatNumber(_medication.lowStockThreshold)} ${_medication.stockUnit}'),
            ...
            const SizedBox(height: 12), Row(children: [Expanded(child: FilledButton.icon(onPressed: _addStock, icon: const Icon(Icons.add), label: const Text('إضافة مخزون'))), const SizedBox(width: 8), Expanded(child: OutlinedButton.icon(onPressed: _editSettings, icon: const Icon(Icons.settings), label: const Text('الإعدادات')))]),
          ]))),
          const SizedBox(height: 12), MedicationStockHistory(key: _historyKey, medicationId: _medication.id, unit: _medication.stockUnit),
        ],),
      ),
    );
  }
...
class _InfoRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(label), Flexible(child: Text(value, textAlign: TextAlign.end))]));
}
