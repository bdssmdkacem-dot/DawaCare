...
class _MedicationStockDetailPageState extends State<MedicationStockDetailPage> {
  ...
  void _reload() {
    _history = _repo.fetchStockTransactions(_medication.id);
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: InputDecoration(
            labelText: 'الكمية',
            suffixText: _unit(_medication.stockUnit),
          ),
        ),
        actions: [
          TextButton(
  ...
  Future<void> _settings() async {
    ...
    if (!ok) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(provider.error ?? 'تعذر حفظ الإعدادات')),
      );
      return;
    }
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                _medication.stockQuantity <= 0 ? 'نفد الدواء' : 'المخزون الحالي',
                    ...
                    Text(
                      '${_format(_medication.stockQuantity)} ${_unit(_medication.stockUnit)}',
                      style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900, color: color),
                    ),
                    if (days != null)
                      Text(days < 1 ? 'يكفي لأقل من يوم' : 'يكفي تقريبًا لـ ${_format(days)} يوم'),
                    const SizedBox(height: 14),
                    LinearProgressIndicator(
                      minHeight: 9,
                      value: _medication.lowStockThreshold > 0
                          ? (_medication.stockQuantity / (_medication.lowStockThreshold * 3)).clamp(0.0, 1.0).toDouble()
                          : 1.0,
            ...
            Row(
              children: [
                _stat('الحد المنخفض', '${_format(_medication.lowStockThreshold)} ${_unit(_medication.stockUnit)}'),
                _stat('العبوة', _medication.packageQuantity == null ? '—' : '${_format(_medication.packageQuantity!)} ${_unit(_medication.stockUnit)}'),
                _stat('الاستهلاك اليومي', daily > 0 ? _format(daily) : '—'),
              ],
            ),
            const SizedBox(height: 18),
            FilledButton.icon(
            ...
            FutureBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, snapshot) {
                ...
                return Card(
                  child: Column(
                    children: rows.map((row) => _transactionTile(context, row)).toList(),
                  ),
                );
              },
            ),
  ...
  Widget _transactionTile(BuildContext context, Map<String, dynamic> row) {
    ...
    return ListTile(
      leading: CircleAvatar(
        backgroundColor: color.withValues(alpha: .10),
        child: Icon(isOut ? Icons.remove_rounded : Icons.add_rounded, color: color),
      ),
      title: Text(_type(type), style: const TextStyle(fontWeight: FontWeight.w800)),
      subtitle: Text('${_date(created)}${row['note'] == null ? '' : ' • ${row['note']}'}'),
      trailing: Text(
        '${isOut ? '-' : '+'}${_format(quantity)}',
