...
class MessageService {
  ...
  Future<List<ChatMessage>> fetch(String patientId) async {
    final rows=await _client.from('messages').select('*, sender:profiles!sender_id(full_name)').eq('patient_id',patientId).order('created_at');
    return rows.map((r)=>ChatMessage.fromMap(Map<String,dynamic>.from(r))).toList();
  }
  ...
  Future<ChatMessage> sendText({required String patientId,required String recipientId,required String text}) async {
    final sender=_client.auth.currentUser?.id; if(sender==null)throw StateError('AUTH_REQUIRED'); final value=text.trim(); if(value.isEmpty)throw StateError('EMPTY_MESSAGE');
  ...
  Future<ChatMessage> sendImage({required String patientId,required String recipientId,required XFile image}) async {
    final sender=_client.auth.currentUser?.id; if(sender==null)throw StateError('AUTH_REQUIRED');
    final id=_uuid.v4(); final ext=image.name.contains('.')?image.name.split('.').last.toLowerCase():'jpg'; final path='messages/$sender/$id.$ext';
  ...
  Future<ChatMessage> sendVoice({required String patientId,required String recipientId,required String localPath,required int durationMs}) async {
    final sender=_client.auth.currentUser?.id; if(sender==null)throw StateError('AUTH_REQUIRED'); if(durationMs<=0||durationMs>60000)throw StateError('INVALID_VOICE_DURATION');
    final id=_uuid.v4(); final path='messages/$sender/$id.m4a'; await _client.storage.from('message-attachments').upload(path,File(localPath),fileOptions:const FileOptions(contentType:'audio/mp4',upsert:false));
  ...
  Future<ChatMessage> _insert({required String patientId,required String recipientId,required String type,String? body,String? storagePath,String? mimeType,int? durationMs}) async {
    final row=await _client.from('messages').insert({'id':_uuid.v4(),'patient_id':patientId,'sender_id':_client.auth.currentUser!.id,'recipient_id':recipientId,'message_type':type,'body':body,'storage_path':storagePath,'mime_type':mimeType,'duration_ms':durationMs}).select('*, sender:profiles!sender_id(full_name)').single();
    final message=ChatMessage.fromMap(Map<String,dynamic>.from(row));
    try { final r=await _client.functions.invoke('message-notify',body:{'message_id':message.id}); debugPrint('message-notify status=${r.status} data=${r.data}'); } catch(e){debugPrint('message-notify failed: $e');}
    return message;
  }
  ...
  Future<String?> stopVoiceRecording()=>_recorder.stop();
  Future<void> cancelVoiceRecording()=>_recorder.cancel();
  Future<XFile?> pickImage()=>_picker.pickImage(source:ImageSource.gallery,imageQuality:85,maxWidth:1920);
