...
class _VoiceMessagesPageState extends State<VoiceMessagesPage> {
  ...
  Future<void> _selectMessage(String messageId) async {
    final message = _messages.where((m) => m.id == messageId).cast<VoiceMessage?>().firstWhere((m) => m != null, orElse: () => null);
    if (message == null) {
      final fetched = await _service.fetchById(messageId);
  ...
  Future<void> _onPlaybackComplete() async {
    ...
    try {
      ...
      if (result.deleted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تم الاستماع للرسالة مرتين وتم حذفها بأمان.')));
      } else {
        setState(() {
          ...
          if (index >= 0) {
            ...
            _messages[index] = VoiceMessage(
              id: old.id, senderId: old.senderId, patientId: old.patientId, doseId: old.doseId,
              storagePath: old.storagePath, durationMs: old.durationMs, createdAt: old.createdAt,
              readAt: old.readAt, completedListens: result.completedListens, senderName: old.senderName,
            );
          }
        });
      }
    } catch (_) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('اكتمل التشغيل، لكن تعذّر تحديث عداد الاستماع.')));
    }
  }
  ...
  String _listenLabel(VoiceMessage m) => 'الاستماع ${m.completedListens}/2';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : Column(
              children: [
                if (selected != null) _buildContextCard(selected),
                Expanded(
                    ...
                    child: _messages.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return Card(
                                child: ListTile(
                                  leading: CircleAvatar(child: Icon(_playingId == m.id ? Icons.pause_rounded : Icons.play_arrow_rounded)),
                                  title: Text(m.senderName, style: TextStyle(fontWeight: unread ? FontWeight.w800 : FontWeight.w500)),
                                  subtitle: Text('${m.createdAt.hour.toString().padLeft(2, '0')}:${m.createdAt.minute.toString().padLeft(2, '0')}  ${_duration(m.durationMs)}  •  ${_listenLabel(m)}'),
  ...
  Widget _buildContextCard(VoiceMessage message) {
        ...
        child: Row(
          children: [
            if (_contextImageUrl != null && _contextImageUrl!.isNotEmpty)
              ClipRRect(borderRadius: BorderRadius.circular(12), child: Image.network(_contextImageUrl!, width: 64, height: 64, fit: BoxFit.cover))
            else
              const CircleAvatar(radius: 32, child: Icon(Icons.medication_rounded)),
            const SizedBox(width: 12),
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(_contextMedicationName ?? 'رسالة مرتبطة بجرعة', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 17)),
                if (_contextStrength?.isNotEmpty == true) Text(_contextStrength!),
                if (_contextDoseAmount?.isNotEmpty == true) Text('الجرعة: $_contextDoseAmount'),
                if (scheduled != null) Text('وقت الجرعة: ${DateTime.parse(scheduled).hour.toString().padLeft(2, '0')}:${DateTime.parse(scheduled).minute.toString().padLeft(2, '0')}'),
                Text(_listenLabel(message)),
              ]),
            ),
          ],
        ),
