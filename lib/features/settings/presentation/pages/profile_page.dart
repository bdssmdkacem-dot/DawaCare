...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_tr(context, 'الملف الشخصي', 'Profile', 'Profil')),
        centerTitle: false,
      ),
      body: Form(
        key: _formKey,
        child: ListView(
          ...
          children: [
            Container(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    profile.fullName.isEmpty ? l.appName : profile.fullName,
                  ...
                  Text(
                    _tr(context, 'حسابك في DawaCare', 'Your DawaCare account', 'Votre compte DawaCare'),
                    ...
                    TextFormField(
                      ...
                      validator: (value) {
                        if (value == null || value.trim().isEmpty) {
                          return _tr(context, 'أدخل الاسم الكامل.', 'Enter your full name.', 'Saisissez votre nom complet.');
                        }
                        return null;
                      },
                    ),
            ...
            _infoCard(
              ...
              _tr(context, 'رمز العائلة', 'Family code', 'Code famille'),
              profile.familyCode.isEmpty ? '—' : profile.familyCode,
              copyValue: profile.familyCode.isEmpty ? null : profile.familyCode,
            ),
            const SizedBox(height: 10),
            _infoCard(
              context,
              Icons.public_rounded,
              _tr(context, 'المنطقة الزمنية', 'Timezone', 'Fuseau horaire'),
              profile.timezone,
            ),
            const SizedBox(height: 24),
            FilledButton.icon(
  ...
  Widget _infoCard(BuildContext context, IconData icon, String title, String value, {String? copyValue}) {
    return Card(
      elevation: 0,
      child: ListTile(
            ...
            : IconButton(
                ...
                onPressed: () async {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(_tr(context, 'تم نسخ رمز العائلة.', 'Family code copied.', 'Code famille copié.'))),
                  );
                },
              ),
      ),
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_tr(context, 'تم حفظ الملف الشخصي بنجاح.', 'Profile saved successfully.', 'Profil enregistré avec succès.'))),
      );
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_tr(context, 'تعذر حفظ التغييرات. حاول مرة أخرى.', 'Could not save changes. Please try again.', 'Impossible d’enregistrer les modifications. Réessayez.'))),
      );
    } finally {
