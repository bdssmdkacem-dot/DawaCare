...
Deno.serve(async (req) => {
  ...
  const userClient = createClient(URL, Deno.env.get('SUPABASE_ANON_KEY')!, { global: { headers: { Authorization: h } } });
  ...
  const { data: message, error: messageError } = await admin.from('messages').select('id,patient_id,sender_id,recipient_id,message_type,body').eq('id', messageId).single();
  ...
  const { data: link } = await admin.from('caregiver_patient').select('id').eq('patient_id', message.patient_id).or(`caregiver_id.eq.${message.sender_id},caregiver_id.eq.${message.recipient_id}`).maybeSingle();
  if (!link) return json({ error: 'FORBIDDEN' }, 403);
  if (!FCM) return json({ error: 'FCM_NOT_CONFIGURED' }, 500);
  const eventKey = `message:${messageId}`;
  const { data: old } = await admin.from('push_notification_events').select('sent_count').eq('event_key', eventKey).maybeSingle();
  ...
  const valid = (devices ?? []).filter((d) => typeof d.push_token === 'string' && d.push_token.trim());
  ...
  for (const device of valid) {
    try {
      const response = await fetch(`https://fcm.googleapis.com/v1/projects/${account.project_id}/messages:send`, { method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ message: {
  ...
  await admin.from('push_notification_events').upsert({ event_key: eventKey, event_type: 'CHAT_MESSAGE', sender_id: message.sender_id, recipient_id: message.recipient_id, sent_at: sent ? new Date().toISOString() : null, sent_count: sent }, { onConflict: 'event_key' });
  return sent ? json({ sent, devices: valid.length }) : json({ error: 'FCM_DELIVERY_FAILED', sent }, 502);
});
...
async function getAccessToken(a: ServiceAccount): Promise<string> {
  ...
  const b64 = (x: ArrayBuffer | Uint8Array) => { const u = x instanceof Uint8Array ? x : new Uint8Array(x); return btoa(Array.from(u).map((c) => String.fromCharCode(c)).join('')).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); };
  const head = { alg: 'RS256', typ: 'JWT' };
  const claims = { iss: a.client_email, scope: 'https://www.googleapis.com/auth/firebase.messaging', aud: 'https://oauth2.googleapis.com/token', iat: now, exp: now + 3600 };
