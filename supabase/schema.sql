...
-- generate a short, human-shareable family code (e.g. "K7F3QX")
...
-- ----------------------------------------------------------------------------
-- 2. caregiver_patient — links a caregiver profile to a patient profile
-- ----------------------------------------------------------------------------
...
-- helper: is the current user an accepted caregiver of a given patient?
...
-- link a caregiver to a patient by family code, bypassing RLS internally
-- (a caregiver cannot SELECT a stranger's profile directly — that's the
-- point of the code-based handshake instead of an open profile search).
...
begin
  ...
  if found_id is null then
    raise exception 'FAMILY_CODE_NOT_FOUND';
  end if;
...
create table if not exists public.medication_schedules (
  ...
  time           text not null default '08:00', -- "HH:mm" local time
  days_of_week   int[] not null default '{}',   -- 1=Sunday .. 7=Saturday
  interval_days  int,
...
-- ----------------------------------------------------------------------------
-- 5. dose_instances — the generated occurrences of a schedule
-- ----------------------------------------------------------------------------
create table if not exists public.dose_instances (
  ...
  status           text not null default 'PENDING'
                   check (status in ('PENDING', 'REMINDER_SENT', 'SNOOZED', 'TAKEN', 'MISSED', 'SKIPPED', 'CANCELLED')),
  created_at       timestamptz not null default now(),
...
-- ----------------------------------------------------------------------------
-- 6. dose_events — audit trail of every action taken on a dose
-- ----------------------------------------------------------------------------
create table if not exists public.dose_events (
  id           uuid primary key default gen_random_uuid(),
  dose_id      uuid not null references public.dose_instances (id) on delete cascade,
  patient_id   uuid not null references public.profiles (id) on delete cascade,
...
-- ----------------------------------------------------------------------------
-- 7. reminder_policies — one row per patient, tunable escalation rules
-- ----------------------------------------------------------------------------
create table if not exists public.reminder_policies (
  patient_id                 uuid primary key references public.profiles (id) on delete cascade,
  repeat_interval_min        int not null default 15,
  max_repeats                int not null default 2,
  grace_period_min           int not null default 60,
  caregiver_escalation       boolean not null default true,
  updated_at                 timestamptz not null default now()
);

-- ----------------------------------------------------------------------------
-- 8. devices — for push notifications (caregiver alerts)
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 9. caregiver_alerts — generated when a dose is overdue past grace period
-- ----------------------------------------------------------------------------
create table if not exists public.caregiver_alerts (
  ...
  type          text not null default 'MISSED_DOSE',
...
-- profiles: a user can read their own profile, and the profile of any patient
-- they are a caregiver for; can only update their own profile.
create policy "profiles_select_own_or_linked" on public.profiles
  for select using (id = auth.uid() or public.is_caregiver_of(id));
create policy "profiles_update_own" on public.profiles
  for update using (id = auth.uid());

-- caregiver_patient: visible to both sides of the link; only the caregiver
-- (who typed the family code) can create the link; either side can remove it.
create policy "caregiver_patient_select" on public.caregiver_patient
  for select using (caregiver_id = auth.uid() or patient_id = auth.uid());
create policy "caregiver_patient_insert" on public.caregiver_patient
  for insert with check (caregiver_id = auth.uid());
create policy "caregiver_patient_delete" on public.caregiver_patient
  for delete using (caregiver_id = auth.uid() or patient_id = auth.uid());

-- medications / schedules / doses: patient + linked caregivers can view;
-- only patient or an editing-caregiver can write.
create policy "medications_select" on public.medications
  for select using (patient_id = auth.uid() or public.is_caregiver_of(patient_id));
create policy "medications_write" on public.medications
  for all using (public.can_edit_patient(patient_id))
  ...
  for select using (
    exists (select 1 from public.medications m
            where m.id = medication_id
              and (m.patient_id = auth.uid() or public.is_caregiver_of(m.patient_id)))
  );
create policy "schedules_write" on public.medication_schedules
  for all using (
    exists (select 1 from public.medications m
            where m.id = medication_id and public.can_edit_patient(m.patient_id))
  )
  with check (
    exists (select 1 from public.medications m
            where m.id = medication_id and public.can_edit_patient(m.patient_id))
...
create policy "doses_select" on public.dose_instances
  for select using (patient_id = auth.uid() or public.is_caregiver_of(patient_id));
create policy "doses_write" on public.dose_instances
  for all using (public.can_edit_patient(patient_id))
  with check (public.can_edit_patient(patient_id));
...
create policy "dose_events_insert" on public.dose_events
  for insert with check (patient_id = auth.uid() or public.is_caregiver_of(patient_id));

create policy "reminder_policies_all" on public.reminder_policies
  for all using (patient_id = auth.uid() or public.is_caregiver_of(patient_id))
  with check (patient_id = auth.uid() or public.can_edit_patient(patient_id));

create policy "devices_own" on public.devices
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());

create policy "caregiver_alerts_select" on public.caregiver_alerts
  for select using (caregiver_id = auth.uid());
create policy "caregiver_alerts_update" on public.caregiver_alerts
  for update using (caregiver_id = auth.uid());
-- inserts into caregiver_alerts are done by the escalation-check Edge Function
-- using the service_role key, which bypasses RLS.

create policy "audit_logs_select" on public.audit_logs
  for select using (actor_id = auth.uid() or public.is_caregiver_of(entity_id));

-- ============================================================================
-- Realtime (so caregivers see dose confirmations live)
-- ============================================================================
